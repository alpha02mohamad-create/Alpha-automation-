...
def _fallback_image() -> str | None:
    """يرجع مسار صورة احتياطية جاهزة لو موجودة، وإلا None (بدل ما يكسر الـ pipeline)."""
...
def _generate_placeholder(out_path: str) -> str:
    """
    احتياطي أخير لو مفيش صور جاهزة في assets/fallback_images/ ولا Cloudflare شغال:
    يولّد صورة عادية بلون واحد بدل ما يوقف الـ pipeline بالكامل. الفيديو هيطلع
    بجودة أقل بصرياً في المشهد ده تحديداً، لكن التشغيل هيكمل لحد النشر.
    """
    color = (random.randint(20, 60), random.randint(20, 60), random.randint(30, 70))
...
def _copy_fallback(out_path: str) -> str:
    ...
    if fallback_path is None:
        print("[image_gen] No fallback images in assets/fallback_images/; "
              "generating a plain placeholder image instead")
        return _generate_placeholder(out_path)
